...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if (isset($_POST['name'])) {
    ...
    $onlogin = ':put (",'.$expmode.',' . $price . ',' . $validity . ','.$sprice.',,' . $getlock . ',"); {:local comment [ /ip hotspot user get [/ip hotspot user find where name="$user"] comment]; :local ucode [:pic $comment 0 2]; :if ($ucode = "vc" or $ucode = "up" or $comment = "") do={ :local date [ /system clock get date ]; :local year; :local month; :if ([:pic $date 4] = "-") do={ :set year [:pic $date 0 4]; :set month [:pic $date 5 7]; } else={ :set year [:pic $date 7 11]; :set month [:pic $date 0 3]; }; /sys sch add name="$user" disable=no start-date=$date interval="' . $validity . '"; :delay 2s; :local exp [ /sys sch get [ /sys sch find where name="$user" ] next-run]; :local getxp [len $exp]; :if ($getxp = 15) do={ :local d [:pic $exp 0 6]; :local t [:pic $exp 7 16]; :local s ("/"); :local exp ("$d$s$year $t"); /ip hotspot user set comment="$exp" [find where name="$user"];}; :if ($getxp = 8) do={ /ip hotspot user set comment="$date $exp" [find where name="$user"];}; :if ($getxp > 15) do={ /ip hotspot user set comment="$exp" [find where name="$user"];}; /sys sch remove [find where name="$user"]';
    ...
    $bgservice = ':local dateint do={ :local montharray ( "jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec" ); :local days; :local month; :local year; :if ([:pic $d 4] = "-") do={ :set year [:pic $d 0 4]; :set month [:pic $d 5 7]; :set days [:pic $d 8 10]; } else={ :set year [:pic $d 7 11]; :local m [:pic $d 0 3]; :local mfind [:find $montharray $m]; :set month ($mfind + 1); :set days [:pic $d 4 6]; }; :if ([:len $month] = 1) do={ :set month ("0" . $month); }; :return [:tonum ("$year$month$days")]; }; :local timeint do={ :local hours [ :pick $t 0 2 ]; :local minutes [ :pick $t 3 5 ]; :return ($hours * 60 + $minutes) ; }; :local date [ /system clock get date ]; :local time [ /system clock get time ]; :local today [$dateint d=$date] ; :local curtime [$timeint t=$time] ; :foreach i in [ /ip hotspot user find where profile="'.$name.'" ] do={ :local comment [ /ip hotspot user get $i comment]; :local name [ /ip hotspot user get $i name]; :local gettime; :if ([:pic $comment 4] = "-") do={ :set gettime [:pic $comment 11 19]; } else={ :set gettime [:pic $comment 12 20]; }; :if (([:pic $comment 3] = "/" and [:pic $comment 6] = "/") or ([:pic $comment 4] = "-" and [:pic $comment 7] = "-")) do={ :local expd [$dateint d=$comment] ; :local expt [$timeint t=$gettime] ; :if (($expd < $today and $expt < $curtime) or ($expd < $today and $expt > $curtime) or ($expd = $today and $expt < $curtime)) do={ [ /ip hotspot user '.$mode.' $i ]; [ /ip hotspot active remove [find where user=$name] ]; }}}';
