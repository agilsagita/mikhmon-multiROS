    ...
    public function write($command, $param2 = true)
    {
        if ($command) {
            ...
            foreach ($data as $com) {
                ...
                // Intercept and translate owner format if it is a text-based month
                if (preg_match('/^(\??=?owner=)([a-z]{3})(\d{4})$/i', $com, $matches)) {
                    $monthMap = array(
                    ...
                    $prefix = $matches[1];
                    $textMonth = strtolower($matches[2]);
                    $year = $matches[3];
                    if (isset($monthMap[$textMonth])) {
                        $com = $prefix . $monthMap[$textMonth] . $year;
                    }
                }
